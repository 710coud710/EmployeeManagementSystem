...
namespace EmployeeManagementSystem
{
    ...
    {
        SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\LUU HUU DUOC\Documents\emloyee.mdf;Integrated Security=True;Connect Timeout=30;Encrypt=False;");
        ...
        private void salary_updateBtn_Click(object sender, EventArgs e)
        {
            if(salary_employeeID.Text == ""
                ...
                || salary_salary.Text == "")
            {
                MessageBox.Show("Vui long nhap du thong tin!", "Error"
                    , MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                DialogResult check = MessageBox.Show("Ban co muon cap nhat nhan vien ID: " 
                    + salary_employeeID.Text.Trim() + "?", "Thong bao"
                    ...
                    if(connect.State == ConnectionState.Closed)
                    {
                        try
                        {
                            ...
                            using(SqlCommand cmd = new SqlCommand(updateData, connect))
                            {
                                ...
                                MessageBox.Show("Cap nhat luong thanh cong!"
