...
namespace EmployeeManagementSystem
{
    public partial class RegisterForm : Form
    {
        SqlConnection connect 
            = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\LUU HUU DUOC\Documents\emloyee.mdf;Integrated Security=True;Connect Timeout=30");
        public RegisterForm()
        {
            InitializeComponent();
        }
                ...
                if(connect.State != ConnectionState.Open)
                {
                    try
                    {
                        ...
                        using(SqlCommand checkUser = new SqlCommand(selectUsername, connect))
                        {
                            ...
                            if(count >= 1)
                            {
                                MessageBox.Show(signup_username.Text.Trim() + "Tai khoan da ton tai!"
                                    , "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                            else
                            {
                                ...
                                string insertData = "INSERT INTO users " +
                                    "(username, password, date_regiter) " +
